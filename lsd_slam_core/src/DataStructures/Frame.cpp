...
void Frame::require(int dataFlags, int level)
{
  ...
  if (((dataFlags & IDEPTH) && !data.idepthValid[level]) || ((dataFlags & IDEPTH_VAR) && !data.idepthVarValid[level]))
  {
...
void Frame::buildImage(int level)
{
  ...
  if (width % 8 == 0)
  {
    ...
    __asm__ __volatile__("vldmia %[p025], {q10}                        \n\t"  // p025(q10)
                         ...
                         : /* outputs */[ cur_px ] "+&r"(cur_px), [ next_row_px ] "+&r"(next_row_px),
                           [ width_iteration_count ] "+&r"(width_iteration_count),
                           [ height_iteration_count ] "+&r"(height_iteration_count), [ dest ] "+&r"(dest)
